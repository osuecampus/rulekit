 ...
 */
export const syncRules = async (rulekitRoot, targetPath, stack) => {
 ...
 */
export const syncPrompts = async (rulekitRoot, targetPath, stack) => {
 ...
 */
const findAgentsMdFiles = async (dir) => {
    const files = new Map();
 ...
 */
const mergeContent = (commonContent, stackContent) => {
    if (!commonContent && !stackContent)
        return '';
    if (!commonContent)
        return stackContent;
    if (!stackContent)
